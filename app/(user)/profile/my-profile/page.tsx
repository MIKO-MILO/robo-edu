...
export default function MyProfilePage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        // Handle unauthenticated — redirect to login instead of showing an alert
        if (response.status === 401) {
          window.location.href = "/login?next=/profile/my-profile";
          return;
        }

        // Guard against empty body (e.g. network-level redirect) before parsing
        const text = await response.text();
        if (!text) throw new Error("Respons server kosong.");
        const result = JSON.parse(text) as { success: boolean; message?: string; data?: Profile };
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const text = await response.text();
      const result = text ? (JSON.parse(text) as { success: boolean; message?: string; data?: Profile }) : {};
      if (!response.ok) throw new Error((result as { message?: string }).message ?? "Gagal menyimpan profil.");
      setProfile((result as { data: Profile }).data); alert("Profil berhasil disimpan.");
  ...
  async function handleAvatarChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const text = await response.text();
      const result = text ? (JSON.parse(text) as { success: boolean; message?: string; data?: { avatarUrl: string } }) : {};
      if (!response.ok) throw new Error((result as { message?: string }).message ?? "Gagal mengunggah foto.");
      setProfile((current) => ({ ...current, avatarUrl: (result as { data: { avatarUrl: string } }).data.avatarUrl }));
  ...
  async function deleteAvatar() {
    ...
    try {
      ...
      const text = await response.text();
      const result = text ? (JSON.parse(text) as { success: boolean; message?: string }) : {};
      if (!response.ok) throw new Error((result as { message?: string }).message ?? "Gagal menghapus foto.");
