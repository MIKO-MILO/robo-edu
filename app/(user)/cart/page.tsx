      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 xl:gap-12 items-start">
        ...
        <div className="lg:col-span-8">
          <div className="mb-8">
            ...
            <p className="font-body text-muted-foreground text-sm sm:text-base max-w-xl">
              Playful smart companions, selected for your little explorer to learn, play, and grow.
            </p>
          </div>

          {cartItems.length === 0 ? (
            <div className="bg-white/80 rounded-3xl p-8 text-center border-2 border-dashed border-border-strong my-8">
          ...
          <div className="hidden md:grid grid-cols-12 gap-4 items-center bg-[#FAF6ED] rounded-2xl px-6 py-5 text-xs font-bold text-muted-foreground tracking-wider uppercase font-body pb-5 mb-4">
...
<div className="space-y-4">
  {cartItems.map((item) => (
    <div
      key={item.id}
      className="grid grid-cols-1 md:grid-cols-12 gap-4 py-6 px-6 items-center bg-[#FAF6ED] rounded-2xl shadow-sm transition-all duration-300"
    >
                    {/* Item Image & Info */}
                    <div className="col-span-1 md:col-span-6 flex items-center gap-4">
                      {/* Playful Image Container */}
                      <div
                        className={`w-20 h-20 sm:w-24 sm:h-24 ${item.bgColor} rounded-2xl flex items-center justify-center p-2 border-2 border-white shadow-sm flex-shrink-0 transition-transform duration-300 hover:scale-105`}
                      >
                        <Image
                          src={item.image}
                          alt={item.name}
                      ...
                      {/* Product Names & Delete Button */}
                      <div className="flex-1 min-w-0">
                        <h3 className="font-heading text-base sm:text-lg font-bold text-foreground truncate mb-1">
                          {item.name}
                        </h3>
                        <p className="font-body text-xs sm:text-sm text-muted-foreground mb-2 flex items-center gap-2">
                          <span>{item.category}</span>
                          <span className="inline-block w-1.5 h-1.5 rounded-full bg-border-strong"></span>
                          <span className="font-semibold text-primary-900">{item.age}</span>
                        </p>
                    ...
                    {/* Quantity Selector: Outline circles, quantity in middle */}
                    <div className="col-span-1 md:col-span-3 flex justify-between md:justify-center items-center py-2 md:py-0 border-t border-border/40 md:border-none">
                      <span className="md:hidden font-body text-sm text-muted-foreground font-semibold">Quantity</span>
                      
                      <div className="flex items-center">
                        {/* Minus Button */}
                        <button
                          onClick={() => handleQuantityChange(item.id, -1)}
                          className="w-8 h-8 rounded-full border-2 border-border-strong flex items-center justify-center text-foreground hover:bg-white hover:border-foreground active:scale-90 transition-all duration-200"
                        ...
                        {/* Quantity Value */}
                        <span className="font-body font-bold text-base text-foreground w-10 text-center">
                          {item.quantity}
                        </span>
                    ...
                    <div className="col-span-1 md:col-span-3 flex justify-between md:justify-end items-center py-2 md:py-0 border-t border-border/40 md:border-none">
                      <span className="md:hidden font-body text-sm text-muted-foreground font-semibold">Subtotal</span>
              ...
              <div className="pt-6 border-t border-border flex justify-start">
                <Link
                  href="/"
                  className="inline-flex items-center font-body text-sm font-bold text-primary hover:text-primary-700 transition-colors duration-200 group"
                >
                  <svg
        ...
        <div className="lg:col-span-4 lg:sticky lg:top-24">
          <div className="bg-[#FAF6ED] rounded-3xl p-6 sm:p-8 shadow-sm">
            <h2 className="font-heading text-xl sm:text-2xl font-extrabold text-foreground mb-6 pb-2 border-b border-border">
              Order Summary
            </h2>

            <div className="space-y-4 mb-6">
              {/* Subtotal */}
              <div className="flex justify-between font-body text-sm sm:text-base text-foreground">
                <span>Subtotal</span>
              ...
              {appliedDiscount > 0 && (
                <div className="flex justify-between font-body text-sm sm:text-base text-success font-semibold">
              ...
              <div className="border-t border-border pt-4">
                ...
                <div className="flex justify-between items-end">
                  <span className="font-heading text-base sm:text-lg font-bold text-foreground">Total</span>
                  <span className="font-heading text-2xl sm:text-3xl font-extrabold text-primary-900 leading-none">
                    ${total.toFixed(2)}
                  </span>
                </div>
              </div>
            ...
            <button
              ...
              className="w-full font-body font-extrabold text-white bg-[#2483D0] hover:bg-primary-600 px-6 py-4 rounded-full transition-all duration-300 transform hover:scale-[1.02] active:scale-95 shadow-md flex items-center justify-center gap-2 mb-4 cursor-pointer text-base sm:text-lg"
            >
              Proceed to Checkout
              <svg
            ...
            {/* Payment Icons */}
            <div className="flex justify-center items-center gap-4 py-2 opacity-70 hover:opacity-100 transition-opacity duration-300">
              {/* Visa */}
              <svg className="w-8 h-5 text-muted-foreground" viewBox="0 0 48 30" fill="currentColor">
          ...
          <div className="mt-4 bg-[#FAF6ED] rounded-3xl p-5 border border-border flex items-start gap-3">
            <svg
      ...
      {/* RECOMMENDATIONS SECTION (Image 3 layout / slider) */}
      <div className="mt-16 pt-12 border-t border-border-strong/50">
        
        {/* Recommendations Header with Slider Buttons */}
     <div className="flex justify-between items-center mb-8">
  {/* Title + Line */}
 <div className="relative">
  <h2 className="font-heading text-2xl sm:text-3xl font-extrabold text-foreground tracking-tight pb-5">
    You might like
  </h2>
...
/>
  ...
  <div className="flex items-center gap-6 ">
    <button
      ...
      className="w-10 h-10 rounded-full border-2 border-border-strong flex items-center justify-center bg-white text-foreground hover:border-foreground hover:bg-muted active:scale-90 transition-all duration-200"
    ...
    >
      ...
      >
        ...
        <div
          ...
          className="flex gap-6 overflow-x-auto pb-6 pt-2 scrollbar-none scroll-smooth snap-x snap-mandatory"
        >
          {recommendedProducts.map((prod) => {
            ...
            return (
              <div
                key={prod.id}
                className="w-72 flex-shrink-0 bg-white rounded-3xl p-4 shadow-sm border border-border hover:shadow-md hover:scale-[1.01] transition-all duration-300 relative snap-start group"
              >
                ...
                <button
                  ...
                  className="absolute top-6 right-6 z-10 w-9 h-9 rounded-full bg-white flex items-center justify-center shadow-md border border-border hover:scale-105 active:scale-90 transition-transform duration-200 cursor-pointer"
                  aria-label="Toggle wishlist"
                >
                  {isWishlisted ? (
                    <svg className="w-5 h-5 text-danger-bg fill-danger" viewBox="0 0 24 24">
                      <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                    </svg>
                  ) : (
                    <svg className="w-5 h-5 text-muted-foreground hover:text-danger transition-colors duration-200" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute bottom-3 right-3 w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center shadow-lg hover:bg-primary-600 hover:scale-110 active:scale-95 transition-all duration-200 cursor-pointer"
                ...
                <div className="mt-4">
                  <h3 className="font-body text-base font-bold text-foreground group-hover:text-primary transition-colors duration-200 truncate">
                    {prod.name}
                  </h3>
                    ...
                    <div className="flex items-center gap-1.5">
                      {prod.colors.map((color, colorIdx) => (
                        <button
                          ...
                          className={`w-3.5 h-3.5 rounded-full border shadow-sm transition-all duration-200 ${
